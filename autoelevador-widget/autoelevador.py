...
LOG_PATH = os.path.join(config_dir(), "autoelevador.log")


def log(msg):
    """Escribe en %APPDATA%\\AutoelevadorPicking\\autoelevador.log"""
    try:
...
def poll_loop():
    ...
    while not stop_event.is_set():
        try:
            ...
            api_ok = False
            estado = ("error", str(e))
        if estado != last:            # solo loguea cuando cambia, no satura
            log("API %s -> pendientes=%s" % (estado[0], estado[1]))
            last = estado

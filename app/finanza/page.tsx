...
import {
  LayoutDashboard, Globe, Building2, ClipboardList, Landmark, Wallet, TrendingUp, Receipt,
...
import {
  CtasCtesTab, ComexTab, ProveedoresTab, PresupuestosTab, ImpuestosTab, PrestamosTab, CashTab, MacroTab,
...
export default function FinanzaPage() {
  const { data, macro, hydrated, uploading, error, upload, loadMacro, clear } = useFinanzaData();
  const [tab, setTab] = useState<TabId>("ctasctes");
  const [dragging, setDragging] = useState(false);
  const dragCount = useRef(0);

  useEffect(() => { if (tab === "macro" && !macro) loadMacro(); }, [tab, macro, loadMacro]);

  const onDrop = useCallback((e: React.DragEvent) => {
    e.preventDefault(); dragCount.current = 0; setDragging(false);
    const f = Array.from(e.dataTransfer.files).find((x) => x.name.match(/\.(xlsx|xls)$/i));
    if (f) upload(f);
  }, [upload]);

  const tcRef = macro?.dolares.find((d) => /oficial/i.test(d.nombre))?.venta ?? null;
      ...
      {dragging && (
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-4 border-2 border-dashed border-yellow-400 rounded-2xl px-16 py-12">
            ...
            <p className="text-lg font-semibold text-zinc-100">Soltá el Excel para actualizar</p>
            <p className="text-sm text-zinc-500">Funciona en cualquier pestaña</p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center gap-2">
          {data?.periodo && (
            <span className="text-xs text-zinc-300 bg-zinc-800 border border-zinc-700 rounded-full px-4 py-1.5">
              Período: <span className="text-yellow-400 font-semibold">{fmtMes(data.periodo.slice(0, 7))}</span>
            </span>
          )}
          {tcRef != null && (
            <span className="text-xs text-zinc-300 bg-zinc-800 border border-zinc-700 rounded-full px-4 py-1.5">
              TC Ref: <span className="text-yellow-400 font-semibold">{fmtArs(tcRef)}</span>
            </span>
          )}
          {data && (
            <button onClick={clear} title="Limpiar datos" className="text-zinc-600 hover:text-red-400 transition-colors p-2">
              <Trash2 size={16} />
            </button>
          )}
        </div>
        ...
        {TABS.map(({ id, label, icon: Icon }) => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-4 text-sm font-medium border-b-[3px] whitespace-nowrap transition-all duration-100 ${
              tab === id ? "text-yellow-400 border-yellow-400" : "text-zinc-500 border-transparent hover:text-zinc-200"
            }`}
          >
            <Icon size={15} /> {label}
          </button>
        ...
        {!hydrated ? null : !data && tab !== "macro" ? (
          <div className="flex flex-col items-center justify-center py-28 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-zinc-600 text-sm mt-1">Arrastrá el Excel financiero a cualquier parte de esta sección.</p>
            </div>
          </div>
        ) : (
          <>
            ...
            {tab === "proveedores" && data && <ProveedoresTab d={data.proveedores} />}
            {tab === "presupuestos" && data && <PresupuestosTab d={data.presupuestos} />}
            ...
            {tab === "macro" && <MacroTab macro={macro} onRefresh={loadMacro} />}
          </>
        )}
      </main>
    </div>

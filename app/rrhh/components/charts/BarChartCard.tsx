...
function BarChartCard({
  ...
  ubicacionLabel = "top", labelFontSize = 12, labelFill = t.text, xTickFontSize = 12, xAngle = -45,
}: Props) {
  return (
    <>
      <CardTitle className="mb-4 text-lg font-semibold" style={{ color: t.text }}>{title}</CardTitle>

      <ResponsiveContainer width="100%" height={height}>
        <BarChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 60 }}>
          <CartesianGrid strokeDasharray="3 3" stroke={t.grid} />
          <XAxis dataKey={xKey} stroke={t.textMuted} tick={{ fontSize: xTickFontSize }} angle={xAngle} textAnchor="end" height={80} />
          ...
          <Bar dataKey={yKey} fill={t.primary} radius={[4, 4, 0, 0]}>
            <LabelList dataKey={yKey} position={ubicacionLabel} fill={labelFill} fontSize={labelFontSize} fontWeight={600} formatter={(value) => fmtARS(Number(value))} />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </>

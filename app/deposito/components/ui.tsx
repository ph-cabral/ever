...
export const PALETTE = ["#facc15", "#3fb950", "#f0883e", "#58a6ff", "#bc8cff", "#f85149", "#e3b341", "#56d4dd", "#db61a2", "#7ee787"];
...
export const fmtShort = (n: number | null | undefined, prefix = "$") => {
  ...
  const a = Math.abs(n);
  if (a >= 1e9) return `${prefix}${(n / 1e9).toLocaleString("es-AR", { maximumFractionDigits: 1 })}MM`;
  if (a >= 1e6) return `${prefix}${(n / 1e6).toLocaleString("es-AR", { maximumFractionDigits: 1 })}M`;
  if (a >= 1e3) return `${prefix}${(n / 1e3).toLocaleString("es-AR", { maximumFractionDigits: 0 })}K`;
...
export function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 mt-7 mb-3">
      <span className="text-[13px] font-semibold text-zinc-100">{children}</span>
...
export function Panel({
  title, accent, children, className = "", bodyClass = "p-4",
}: { title?: React.ReactNode; accent?: React.ReactNode; children: React.ReactNode; className?: string; bodyClass?: string }) {
  return (
    <div className={`rounded-lg bg-[#171717] border border-zinc-800 ${className}`}>
      {title && (
        <div className="px-4 pt-4 pb-2 mb-1 border-b border-zinc-800 mx-4">
          <h3 className="text-[11px] font-semibold uppercase tracking-wider text-zinc-500">
            {title} {accent && <span className="text-yellow-400 normal-case">{accent}</span>}
          </h3>
        </div>
      )}
...
const ACCENT_TEXT: Record<Accent, string> = {
  yellow: "text-yellow-400", green: "text-green-400", red: "text-red-400", amber: "text-amber-400", neutral: "text-zinc-100",
};
export function KPI({ label, value, sub, accent = "yellow" }: { label: string; value: string; sub?: string; accent?: Accent }) {
  return (
    <div className="rounded-lg bg-[#171717] border border-zinc-800 px-4 py-3.5">
      <p className="text-[11px] uppercase tracking-wider text-zinc-500">{label}</p>
      <p className={`text-[22px] leading-none font-bold mt-1.5 ${ACCENT_TEXT[accent]}`}>{value}</p>
      {sub && <p className="text-[11px] text-zinc-600 mt-1.5 leading-tight">{sub}</p>}
    </div>
  );
}

// ─── Grid ────────────────────────────────────────────────────────────────────
export function Grid({ children, cols = 3 }: { children: React.ReactNode; cols?: 2 | 3 | 4 | 5 | 6 }) {
  const c =
...
const TONE_TEXT: Record<Tone, string> = {
  green: "text-green-400", amber: "text-amber-400", orange: "text-orange-400", red: "text-red-400", yellow: "text-yellow-400", neutral: "text-zinc-300",
};
export function StatBox({ big, label, value, tone = "neutral" }: { big: string; label: string; value?: string; tone?: Tone }) {
  return (
...
export function Tag({ children, tone = "neutral" }: { children: React.ReactNode; tone?: Tone }) {
  return <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold ${TAG[tone]}`}>{children}</span>;
}

// ─── Progress bar ─────────────────────────────────────────────────────────────
export function Progress({ label, pct, value, tone = "yellow", labelMin = 160 }: { label: string; pct: number; value?: string; tone?: Tone; labelMin?: number }) {
  const bar: Record<Tone, string> = {
    green: "bg-green-400", amber: "bg-amber-400", orange: "bg-orange-400", red: "bg-red-400", yellow: "bg-yellow-400", neutral: "bg-zinc-500",
  };
  return (
    <div className="flex items-center gap-2 mb-2">
      <div className="text-[11px] text-zinc-500 truncate" style={{ minWidth: labelMin }}>{label}</div>
      <div className="flex-1 h-1.5 bg-[#1f1f1f] rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${bar[tone]}`} style={{ width: `${Math.max(0, Math.min(100, pct))}%` }} />
      </div>
      {value && <div className={`text-[11px] tabular-nums text-right ${TONE_TEXT[tone]}`} style={{ minWidth: 80 }}>{value}</div>}
    </div>
  );
}

// ─── Alert ────────────────────────────────────────────────────────────────────
export function Alert({ tone = "yellow", children }: { tone?: Tone; children: React.ReactNode }) {
  ...
  return <div className={`rounded-md border px-3.5 py-2.5 text-[12px] flex items-start gap-2 ${m[tone]}`}>{children}</div>;
}

// ─── Tabla genérica ───────────────────────────────────────────────────────────
export interface Col<T> { key: string; label: string; num?: boolean; render?: (row: T) => React.ReactNode; className?: string }
export function Table<T>({ cols, rows, max = 200, empty = "Sin datos", maxH }: { cols: Col<T>[]; rows: T[]; max?: number; empty?: string; maxH?: number }) {
  const shown = rows.slice(0, max);
  return (
    <div className="rounded-lg bg-[#171717] border border-zinc-800 overflow-auto" style={maxH ? { maxHeight: maxH } : undefined}>
      <table className="w-full text-[12px]">
        <thead className="sticky top-0 z-10">
          <tr className="bg-[#1f1f1f]">
            {cols.map((c) => (
              <th key={c.key} className={`px-2.5 py-2 text-[10px] font-semibold uppercase tracking-wider text-zinc-500 whitespace-nowrap border-b border-zinc-800 ${c.num ? "text-right" : "text-left"}`}>
                {c.label}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {shown.map((row, i) => (
            <tr key={i} className="border-b border-zinc-800/60 hover:bg-[#1f1f1f] transition-colors">
              {cols.map((c) => (
                <td key={c.key} className={`px-2.5 py-1.5 whitespace-nowrap ${c.num ? "text-right tabular-nums text-zinc-200" : "text-zinc-300"} ${c.className ?? ""}`}>
      ...
      {rows.length > max && <div className="px-3 py-1.5 text-[11px] text-zinc-600 border-t border-zinc-800">Mostrando {max} de {rows.length}</div>}
    </div>
  );
}
...
export function ChartLine({
  data, xKey, series, height = 220, fmt = (n) => fmtShort(n),
}: { data: unknown[]; xKey: string; series: Serie[]; height?: number; fmt?: (n: number) => string }) {
  if (!data.length) return <Empty h={height} />;
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 4 }}>
        ...
        {series.map((s, i) => (
          <Line key={s.key} type="monotone" dataKey={s.key} name={s.name} stroke={s.color ?? PALETTE[i % PALETTE.length]} strokeWidth={2} dot={{ r: 2 }} activeDot={{ r: 4 }} />
        ))}
      </LineChart>
    </ResponsiveContainer>
  );
...
export function ChartDonut({
  data, height = 220, fmt = (n) => fmtShort(n),
}: { data: { name: string; value: number; color?: string }[]; height?: number; fmt?: (n: number) => string }) {
  const clean = data.filter((d) => d.value > 0);
  if (!clean.length) return <Empty h={height} />;
  return (
    <ResponsiveContainer width="100%" height={height}>
      <PieChart>
        <Pie data={clean} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius="55%" outerRadius="80%" paddingAngle={2} stroke={C.surface}>
          {clean.map((d, i) => <Cell key={i} fill={d.color ?? PALETTE[i % PALETTE.length]} />)}
        </Pie>
        <Tooltip contentStyle={tooltipStyle} formatter={(v) => fmt(Number(v))} />
        <Legend wrapperStyle={{ fontSize: 11, color: C.muted }} />
      </PieChart>
...
function Empty({ h }: { h: number }) {
  return <div className="flex items-center justify-center text-zinc-700 text-xs" style={{ height: h }}>Sin datos para graficar</div>;
}

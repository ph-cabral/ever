...
import { ResumenTab, ProcesoTab, OperariosTab, TiempoTab } from "./components/tabs";
    ...
    >
      {dragging && (
        <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm flex items-center justify-center pointer-events-none">
          <div className="flex flex-col items-center gap-4 border-2 border-dashed border-yellow-400 rounded-2xl px-16 py-12">
            ...
            <p className="text-lg font-semibold text-zinc-100">Soltá el archivo para actualizar</p>
            <p className="text-sm text-zinc-500">Producción (prod.csv) o Tiempo de Pedidos (.xlsx) — se detecta solo</p>
          </div>
        </div>
      )}
        ...
        <div className="fixed bottom-6 right-6 z-[110]">
          {uploading && (
            <div className="flex items-center gap-3 bg-[#1A1A1A] border border-yellow-400/40 rounded-xl px-5 py-3 text-sm text-zinc-200">
              <Loader2 size={16} className="animate-spin text-yellow-400" /> Procesando archivo…
            </div>
          )}
          {!uploading && error && (
      ...
      <header className="sticky top-0 z-50 bg-[#1A1A1A] border-b-[3px] border-yellow-400 flex items-center justify-between px-8 h-16">
        <div className="flex items-center gap-4">
          ...
          <span className="text-zinc-500 text-sm">Depósito · Producción WMS</span>
        </div>
        <div className="flex items-center gap-2">
          {prod?.resumen.nombreUltimoMes && (
            <span className="text-xs text-zinc-300 bg-zinc-800 border border-zinc-700 rounded-full px-4 py-1.5">
              Prod.: <span className="text-yellow-400 font-semibold">{prod.resumen.nombreUltimoMes}</span>
            </span>
          )}
          {tiempo?.mesReciente && (
            <span className="text-xs text-zinc-300 bg-zinc-800 border border-zinc-700 rounded-full px-4 py-1.5">
              Tiempos: <span className="text-yellow-400 font-semibold">{tiempo.mesReciente}</span>
            </span>
          )}
          {(prod || tiempo) && (
      ...
      <main className="max-w-[1400px] mx-auto px-8 py-8">
        {!hydrated ? null : !ready ? (
          <div className="flex flex-col items-center justify-center py-28 gap-4 text-center">
            ...
            <div>
              <p className="text-zinc-400 font-medium">
                {needs === "tiempo" ? "No hay datos de tiempos cargados" : "No hay datos de producción cargados"}
              </p>
          ...
          <>
            {tab === "resumen" && prod && <ResumenTab d={prod} />}
            {tab === "picking" && prod && <ProcesoTab d={prod} proceso="Picking" />}
            {tab === "librepo" && prod && <ProcesoTab d={prod} proceso="Libre + Reposicion" />}
            {tab === "reub" && prod && <ProcesoTab d={prod} proceso="Re-Ubicacion" />}
            {tab === "operarios" && prod && <OperariosTab d={prod} />}
            {tab === "tiempo" && tiempo && <TiempoTab d={tiempo} />}
          </>
        )}
      </main>
    </div>

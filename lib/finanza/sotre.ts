...
export interface DolarTipo { nombre: string; compra: number | null; venta: number | null }
...
interface Persisted { data: FinanzaData | null; macro: MacroData | null }
...
function load(): Persisted {
  ...
  } catch { return { data: null, macro: null }; }
}
function save(p: Persisted) {
  if (typeof window === "undefined") return;
  try { localStorage.setItem(KEY, JSON.stringify(p)); } catch (e) { console.error("finanza persist:", e); }
}
...
export function useFinanzaData() {
  ...
  useEffect(() => {
    const p = load(); setData(p.data); setMacro(p.macro); setHydrated(true);
  }, []);
  useEffect(() => { if (hydrated) save({ data, macro }); }, [data, macro, hydrated]);

  const upload = useCallback(async (file: File) => {
    if (!file.name.match(/\.(xlsx|xls)$/i)) { setError("Archivo no es Excel"); return; }
    setUploading(true); setError(null);
    try {
      const fd = new FormData(); fd.append("file", file);
      const res = await fetch("/api/finanza/parse", { method: "POST", body: fd });
  ...
  const clear = useCallback(() => {
    setData(null);
    if (typeof window !== "undefined") { const p = load(); save({ data: null, macro: p.macro }); }
  }, []);
